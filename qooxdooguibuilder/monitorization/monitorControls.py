...
class MonitorControls(QtCore.QObject):
	...
	def __init__(self):
		...
		#Ler propriedades
		self.loadPropertiesFromFile(DIR_DATA+FILE_CONTROLS_PROPERTIES)
		self.loadGlobalProperties(DIR_DATA+FILE_GLOBAL_PROPERTIES)
	...
	def loadPropertiesFromFile(self, filePath):
		...
		try:
			...
			for line in fcontrols:
				...
				line = line.split(':')		
				typeControl = line[0]
				filePathTypeControl = DIR_DATA+line[1]
	...
	def setSelectedControl(self, typeControl, idControl, unSelectedOtherControls = true):		
		...
		self.LControlsSelected.append(self.controlSelected.copy())
